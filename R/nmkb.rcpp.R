...
#' gcpp
#' Helper function; replaces the function nested into `nmkb` and is hopefully
#' faster.
cppFunction('NumericVector gcpp(NumericVector x, 
                              LogicalVector c1, LogicalVector c3, LogicalVector c4, 
  ...
  for(int i = 0; i < n; ++i) {
    if (c1[i]) {
      gx[i] = atanh( 2 * (x[i] - lower[i]) / (upper[i] - lower[i]) - 1 );
    } else if (c3[i]) {
      gx[i] = log(x[i] - lower[i]);
    } else if (c4[i]) {
      gx[i] = log(upper[i] - x[i]);
    }
  }
  return gx;
}')
...
#' Helper function; replaces the function nested into `nmkb` and is hopefully
#' faster.
cppFunction('NumericVector ginvcpp(NumericVector x, 
                              LogicalVector c1, LogicalVector c3, LogicalVector c4, 
                              NumericVector lower, NumericVector upper) {
  int n = x.size();
  NumericVector gx(n);
  ...
  for(int i = 0; i < n; ++i) {
    if(c1[i]) {
      gx[i] = lower[i] + (upper[i] - lower[i])/2 * (1+ tanh(x[i]));
    } else if (c3[i]) {
      gx[i] = lower[i] + exp(x[i]);
    } else if (c4[i]) {
      gx[i] = upper[i] - exp(x[i]);
    }
  }
  return gx;
}')
...
#' for optimization". It is implemented here with the permission of Prof. Kelley 
#' and SIAM. However, there are some non-trivial modifications of the algorithm 
#' made by Ravi Varadhan.
nmkb <- function (par, fn, lower = -Inf, upper = Inf, control = list(), ...) 
{
  ...
  ginv <- function(x) {
    ...
    # gix
    ginvcpp(x, c1, c3, c4, lower, upper)
  }
